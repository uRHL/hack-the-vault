from htv.utils import Cache, FsTools, check_updates
...
class TestConf:
    ...
    def test_init(self):
        # By default 6 values are loaded: 2 default params, 4 runtime params
        assert len(CONF) == 6
...
class TestCache:
    ...
    def test_clear(self):
        Cache.clear()
        assert not CONF['_CACHE_PATH'].exists()
    ...
    def test_set(self):
        ...
        assert CONF['_CACHE_PATH'].exists()
...
class TestFsTools:
    ...
    def test_dump_file_overwrite(self):
        ...
        FsTools.dump_file(self.test_file, 'Lorem ipsum', exist_ok=True)
        assert self.test_file.exists()
        self.test_file.unlink()
